...
RPC_perform_iteration <- function(df, expl_vars, time_col, censor_col, beta, unique_event_times) {
    ...
    for (i in 1:D) {
      ...
      #if (length(z_ebz)<2){
      #  print(i)
      #  print(paste("z_ebz",length(z_ebz)))}
        #write.csv(data,"C:\\Users\\P70070487\\Downloads\\Radiomics\\2022_montreal2_.csv")
...
call <- function(client, method, ...) {
  ...
  task = list(
    ...
    #"image"="varshagouthamchand/dl_cox_radiomics",
    "collaboration_id"=client$get("collaboration_id"),
    "input"=input,
    "description"="",
    "organization_ids"=list(c(1), c(3))
  )
  ...
  for (k in 1:length(sites)) {
    #print(textConnection(sites[[k]]$result))
...
dcoxph <- function(client, expl_vars, time_col, censor_col, call.method=call) {
  ...
  #beta <- beta_old <- runif(n=m, min = 0, max = 5)
  delta <- 0
